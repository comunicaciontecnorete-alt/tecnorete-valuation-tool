  ...
  return (
    <header className="border-b border-border bg-surface">
      <MotionReveal
        y={8}
        duration={0.4}
        className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-5 py-4 sm:gap-4 md:px-8"
      >
        <Link
        ...
        >
          <Image
            ...
            width={1582}
            height={332}
            sizes="(max-width: 639px) 134px, (max-width: 767px) 153px, 172px"
        ...
        {siteConfig.offices && siteConfig.offices.length > 0 && (
          <div className="flex min-w-0 flex-shrink flex-wrap items-center justify-end gap-1.5 sm:gap-2">
            {siteConfig.offices.map((office) => (
              <div
                key={office.phone}
                className="inline-flex items-center gap-0.5 rounded-full bg-brand-orange px-2 py-2 text-xs font-bold text-white transition hover:-translate-y-px hover:opacity-90 sm:gap-1 sm:px-3 md:px-5 md:text-sm"
              >
                <a
                  href={`https://wa.me/34${office.phone}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Abrir WhatsApp de ${office.headerLabel}`}
                  className="inline-flex flex-shrink-0 items-center justify-center rounded-full active:scale-[0.98] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
                >
                  <svg
                    aria-hidden="true"
                    viewBox="0 0 24 24"
